...
============================================================
           ...
           characteristics, eqc, expf / L, L_B

OK!!, COMPLETE FOR JCT, APART FROM DIMENSION one
      (SYMMETRIC RESULTS)...
...
Lemma expf_L0_CNS:forall (m:fmap)(x y z t:dart),
  inv_hmap (L m zero x y) -> exd m z ->
  (expf (L m zero x y) z t <->
   let x0 := cA m zero x in
   let x_1 := cA_1 m one x in
...
(* SYMMETRICALLY, FOR DIMENSION 1, WE HAVE:
TAKEN AS AXIOMS, USEFUL IN nf_L0L1... *)

Lemma expf_L1_CS:forall (m:fmap)(x y z t:dart),
  inv_hmap (L m one x y) -> exd m z ->
    let x1 := cA m one x in
    let x10 := cA m zero x1 in
    let y0 := cA m zero y in   
    let y_1 := cA_1 m one x in
     (if expf_dec m x y0 
...
Lemma expf_L1_CN:forall (m:fmap)(x y z t:dart),
  inv_hmap (L m one x y) -> exd m z ->
  expf (L m one x y) z t -> 
    let x1 := cA m one x in
    let x10 := cA m zero x1 in 
...
(* SYMMETRICALLY, FOR L1, NOT USED IN JCT ?: 

Lemma expf_not_expf_L1: forall (m:fmap)(x y:dart),
  inv_hmap (L m one x y) -> 
  let y_1:= cA_1 m one y in
  let y0 := cA m zero y in
   (expf m x y0 <-> 
      ~expf (L m one x y) x y_1).
Admitted.
...
(* ANCIENT FORMULATION: USEFUL? 

Lemma between_expf_L0_4:forall(m:fmap)(x y:dart)(i:nat),
  inv_hmap (L m zero x y) -> 
   let x_1 := cA_1 m one x in 
   let y_0 := cA_1 m zero y in 
   let y_0_1 := cA_1 m one y_0 in
...
Proof.
intros.
generalize (between_expf_L0_4_prel m x y i H H0 H1).
fold y_0 in |- *.
fold y_0_1 in |- *.
fold z in |- *.
intro.
...
generalize (H1 H3 H8).
 ...
 split.
   tauto.
  omega.
assert (expf (L m zero x y) y_0_1 y).
 assert (inv_hmap (L m zero x y)).
  simpl in |- *.
    unfold prec_L in |- *.
     tauto.
 generalize (between_expf_L0_4_prel m x y j H15 H0).
   intros.
   fold y_0 in H16.
   fold y_0_1 in H16.
   rewrite <- H in H16.
